...
export default function TipsPage() {
  ...
  startOfWeek.setDate(
    nowLocal.getDate() - nowLocal.getDay() + (nowLocal.getDay() === 0 ? -6 : 1),
  );
  ...
  const totalReceived = filteredTips
    .filter((t) => t.status === "received")
    .reduce((sum, t) => sum + t.amount, 0);
  const totalPending = filteredTips
        ...
        <div className="font-sans antialiased w-full min-h-screen bg-[#FDF2F8] flex flex-col pb-28 relative overflow-x-hidden md:p-8">
          <div className="md:max-w-4xl md:mx-auto w-full flex flex-col flex-grow">
            ...
            <header className="px-6 pt-10 pb-4 flex items-center justify-between sticky top-0 bg-[#FDF2F8]/90 backdrop-blur-md z-40">
              <div className="flex items-center gap-3">
                <button
                  onClick={() => router.back()}
                  className="w-10 h-10 rounded-full border border-pink-100 bg-white flex items-center justify-center text-[#1E293B] shadow-sm"
                >
                  <svg
                    className="h-5 w-5"
                    fill="none"
              ...
              <section className="mb-4">
                <div className="flex flex-col gap-6">
                  {Object.entries(groupedTips).map(([dateGroup, tipsList]) => {
                    const pendingTips = tipsList.filter(
                      (t) => t.status === "pending",
                    );
                    const dailyTotal = tipsList.reduce(
                      (sum, t) => sum + t.amount,
                      0,
                    );
                    return (
                        ...
                        <div className="flex justify-between items-center mb-3">
                          <div className="flex items-baseline gap-2">
                            ...
                            <span className="text-xs font-bold text-gray-400">
                              Tổng: {dailyTotal.toLocaleString("vi-VN")} đ
                            </span>
                          </div>
                          {pendingTips.length > 0 && (
                            <button
                              onClick={() =>
                                handleOpenSheet(pendingTips.map((t) => t.id))
                              }
                        ...
                        <div className="flex flex-col gap-3">
                          {tipsList.map((tip, index) => (
                            <motion.div
                              ...
                              onClick={() => {
                                setActionMenuId(
                                  actionMenuId === tip.id ? null : tip.id,
                                );
                              }}
                            ...
                            >
                              <div className="flex items-center gap-3">
                                <div
                                  className={`w-10 h-10 rounded-full flex items-center justify-center font-bold text-sm ${tip.type === "nail" ? "bg-pink-100 text-[#EC4899]" : "bg-blue-100 text-blue-500"}`}
                                >
                                  {tip.customerName === "Khách hàng"
                                    ? "K"
                                    : tip.customerName.charAt(0)}
                                </div>
                                <div>
                                  <h3
                                    className={`font-bold text-sm ${tip.customerName === "Khách hàng" ? "text-[#94A3B8]" : "text-[#1E293B]"}`}
                                  >
                                    {tip.customerName}
                                  </h3>
                              ...
                              <div className="flex flex-col items-end gap-1">
                                <div className="flex items-center gap-2">
                                  <span
                                    className={`text-lg font-black ${tip.status === "received" ? "text-green-500" : "text-[#1E293B]"}`}
                                  >
                                        ...
                                        (t) => (
                                          <div className="flex flex-col gap-2">
                                            ...
                                            <span className="text-sm">
                                              Nếu đã nhận, số dư ví sẽ được trừ
                                              lại.
                                            </span>
          ...
          <div className="fixed bottom-24 right-6 md:bottom-8 md:right-8 z-50">
            <Link href="/add-tips">
              <motion.div
                whileTap={{ scale: 0.95 }}
                className="w-14 h-14 bg-[linear-gradient(to_bottom_right,#EC4899,#F43F5E)] rounded-full text-white shadow-lg shadow-pink-300 flex items-center justify-center transition-transform"
              >
                <svg
                  className="h-7 w-7"
                  fill="none"
