...
export default function ReportsPage() {
  ...
  const monthTransactions = useMemo(() => {
    return transactions.filter((t) => {
      ...
      return (
        baseDate.getMonth() + 1 === selectedMonth &&
        baseDate.getFullYear() === selectedYear
      );
    });
  ...
  const totalIncome =
    ...
    monthTips.reduce((s, t) => s + t.amount, 0);
  ...
  return (
    <>
      <PageWrapper>
        <div
          className={`font-sans antialiased w-full min-h-screen ${activeUserId === "kha" ? "bg-[#F0FDF4]" : "bg-[#FDF2F8]"} flex flex-col pb-28 relative overflow-x-hidden md:p-8`}
        >
          {/* HEADER */}
          <header
            className={`px-6 pt-10 pb-4 sticky top-0 ${activeUserId === "kha" ? "bg-[#F0FDF4]/90" : "bg-[#FDF2F8]/90"} backdrop-blur-md z-40 flex items-center gap-3`}
          >
            <button
              onClick={() => router.back()}
              className="w-10 h-10 rounded-full border border-gray-200 bg-white flex items-center justify-center text-[#1E293B]"
            >
              <svg
                className="h-5 w-5"
                fill="none"
                ...
                {Array.from({ length: 12 }, (_, i) => i + 1).map((month) => {
                  ...
                  return (
                    <button
                      ...
                      className={`px-5 py-2 whitespace-nowrap rounded-full font-bold text-sm transition-all duration-300 ${
                        isSelected
            ...
            <div className="px-6">
              ...
              <div className="grid grid-cols-2 gap-4 mb-8">
                <div className="bg-white rounded-[1.5rem] p-4 shadow-sm border border-emerald-50">
                  ...
                  <p className="text-xs text-[#94A3B8] font-bold mb-1">
                    TỔNG THU
                  </p>
              ...
              <div className="flex flex-col md:flex-row gap-8 items-start">
                ...
                <div className="w-full md:w-1/2">
                  {expenseData.length > 0 ? (
                    <div
                      className={`bg-white rounded-[2rem] p-6 shadow-sm border flex flex-col items-center mb-8 md:mb-0 md:h-96 ${activeUserId === "kha" ? "border-emerald-50" : "border-pink-50"}`}
                    >
                      <h2 className="text-sm font-bold text-[#1E293B] self-start mb-4">
                        Cơ cấu Chi tiêu
                      </h2>
                        ...
                        <div className="absolute inset-0 flex flex-col items-center justify-center pointer-events-none">
                          ...
                          <span className="text-[#1E293B] font-black">
                            {totalExpense >= 1000000
                              ? (totalExpense / 1000000).toFixed(1) + "Tr"
                    ...
                    <div
                      className={`bg-white rounded-[2rem] p-6 shadow-sm border flex flex-col items-center justify-center mb-8 h-48 ${activeUserId === "kha" ? "border-emerald-50" : "border-pink-50"}`}
                    >
                      ...
                      <div className="flex flex-col gap-3">
                        {expenseData.map((item, index) => {
                          ...
                          return (
                            <div
                              key={index}
                              className={`bg-white rounded-2xl p-4 shadow-sm border flex items-center justify-between ${activeUserId === "kha" ? "border-emerald-50" : "border-pink-50"}`}
                            >
                              <div className="flex items-center gap-3">
                                <div
                                  className="w-10 h-10 rounded-full flex items-center justify-center text-xl bg-gray-50 border border-gray-100"
                                  style={{
