            ...
            <main className="px-6 mt-4 flex-grow flex flex-col">
                <section className="mb-6">
                    {goals.map(g => {
                        ...
                        return (
                            <div key={g.id} className="bg-white rounded-[1.5rem] shadow-sm border border-pink-50 p-5 mb-4">
                                <div className="flex items-center justify-between mb-4">
                                    ...
                                    <span className="text-[#EC4899] font-bold text-sm">{percent}%</span>
                                </div>
                                <div className="w-full bg-gray-100 rounded-full h-2 mb-2">
                                    <div className="bg-[linear-gradient(to_right,#FF9A9E,#F43F5E)] h-2 rounded-full" style={{ width: `${percent}%` }}></div>
                                </div>
                                <p className="text-xs font-bold text-right text-[#94A3B8]">Đã gom: {g.currentAmount.toLocaleString('vi-VN')} đ</p>
                            </div>
                        );
                    })}
