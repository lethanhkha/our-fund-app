...
export const RecentCard: React.FC<RecentCardProps> = ({ name, description, amount, statusText = 'Đã nhận', avatarColorClass = 'bg-[var(--color-brand-secondary)]' }) => {
    return (
        <div className="bg-white rounded-[1.5rem] p-4 flex items-center justify-between shadow-[0_10px_30px_-10px_rgba(238,43,91,0.1)] mb-3 transition-transform active:scale-[0.98]">
            ...
            <div className="text-right">
                ...
                <span className="text-[10px] px-2 py-0.5 bg-[var(--color-brand-secondary)] text-primary rounded-full font-semibold">{statusText}</span>
            </div>
        </div>
    );
};
